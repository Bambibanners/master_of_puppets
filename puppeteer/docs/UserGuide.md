...
### Key Concepts
*   **Agent**: The central server (Brain) that manages state, jobs, and security.
*   **Node**: A lightweight worker (Satellite) that executes tasks. Nodes are isolated and reach out to the Agent (Pull Model).
...
### Health Centre (Nodes)
The **Nodes** view provides a high-density "Network Operations Center" (NOC) oversight.
*   **Status Indicators**:
    *   🟢 **Green Pulse (Online)**: Node is checking in regularly (<30s).
    *   🔴 **Red (Offline)**: Node has missed heartbeats >30s.
...
*   **Verified Badge**: Indicates the node has successfully presented a valid mTLS certificate.
...
## 3. Node Enrollment (Getting Started)
...
2.  **Run Installer**:
    *   **Windows**:
        ```powershell
        iex (irm https://<server>:8001/api/installer) -Role Node -Token "<your-token>"
        ```
    *   **Docker**:
        Use the `compose` file generated in the Admin UI.

---

## 4. Network Mounts (Host-Passthrough)
MoP allows Nodes to map network drives (SMB/CIFS) securely using the Host's credentials, without storing passwords in the Agent.
...
3.  **Deploy**: The configuration is pushed to nodes primarily via Heartbeat.

---

## 5. Troubleshooting
*   **Node Offline**: Check if the `master_of_puppets_node` container is running (`docker ps`). Check logs: `docker logs master_of_puppets_node_1`.
